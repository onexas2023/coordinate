...
public class MessageboxInputTextDlg extends Window {
	...
	/** The event listener. */
	private EventListener<InputClickEvent<String>> _listener;
	 ...
	 * @since 3.0.4
	 */
	public void setEventListener(EventListener<InputClickEvent<String>> listener) {
		_listener = listener;
	}
	/** Sets the focus.
	...
	public void endModal(Messagebox.Button button) throws Exception {
		_result = button;
		if (_listener != null) {
			Textbox inputTextbox = (Textbox)getFellowIfAny("inputTextbox");
			final InputClickEvent<String> evt = new InputClickEvent<String>(button.event, this, button,
					inputTextbox.getValue());
	...
	public void onClose() {
		if (_listener != null) {
			final InputClickEvent<String> evt = new InputClickEvent<String>(Events.ON_CLOSE, this, null, null);
			try {
				_listener.onEvent(evt);
				if (!evt.isPropagatable())
					return; //no more processing
	...
	public static class Button extends org.zkoss.zul.Button {
		...
		public void onClick() throws Exception {
			((MessageboxInputTextDlg)getSpaceOwner()).endModal(_button);
		}
