...
public class MessageboxInputListDlg<T> extends Window {
	...
	/** The event listener. */
	private EventListener<InputClickEvent<T>> _listener;
	 ...
	 * @since 3.0.4
	 */
	public void setEventListener(EventListener<InputClickEvent<T>> listener) {
		_listener = listener;
	}
	...
	public void endModal(Messagebox.Button button) throws Exception {
		_result = button;
		if (_listener != null) {
			Listbox inputListbox = (Listbox) getFellowIfAny("inputListbox");
			LabelObject<T> selection = (LabelObject<T>)((ListModelList) inputListbox.getListModel())
					.getSingleSelection();
			final InputClickEvent<T> evt = new InputClickEvent<T>(button.event, this, button,
					selection == null ? null : selection.getObject());
	...
	public void onClose() {
		if (_listener != null) {
			final InputClickEvent<T> evt = new InputClickEvent<T>(Events.ON_CLOSE, this, null, null);
			try {
				_listener.onEvent(evt);
				if (!evt.isPropagatable())
					return; // no more processing
	...
	public static class Button extends org.zkoss.zul.Button {
		...
		public void onClick() throws Exception {
			((MessageboxInputListDlg) getSpaceOwner()).endModal(_button);
		}
