 ...
 *
 */
public class GsonsTest extends CoordinateCommonTestBase {
	...
	public void testSimple() {
		...
		attr.put("abc.xyz", 99.9D);
		attr.put("abc.xyz2", 33D);
		p.setAttr(attr);

		String json = Gsons.jsonify(p);
		System.out.println("1>>" + json);

		String plain = json;
		String pretty = Gsons.pretty(json);
		System.out.println("2>>" + pretty);

		String plain2 = Gsons.plain(pretty);
		System.out.println("3>>" + plain2);

		Assert.assertEquals(plain, plain2);
		Assert.assertNotEquals(plain, pretty);

		Person p1 = Gsons.objectify(json, Person.class);
		...
		//in original attr, it is 33D, however after gson map converter, it become 33L
		Assert.assertEquals(((Double)p.getAttr().get("abc.xyz2")).longValue(), (long)p1.getAttr().get("abc.xyz2"));
	}
	...
	public void testConfig() {
		...
		String ymal = Gsons.jsonify(p);
		System.out.println("1>>" + ymal);

		Configuration config = null;
		try {
			config = Gsons.toConfiguration(ymal);
	...
	public void testTransform() {
		...
		Person2 p1Top2 = Gsons.transform(p1, Person2.class);
		Person p2Top1 = Gsons.transform(p2, Person.class);
	...
	public void testGenericMemberField() {
		...
		String json = Gsons.jsonify(foo);
		System.out.println(">>>>" + json);

		foo = Gsons.objectify(json, Foo.class);
	...
	public void testCleanOnErrory() {
		...
		Map map = Gsons.objectify("abc", Map.class, true);
		Assert.assertEquals(0, map.size());

		Person person = Gsons.objectify("abc", Person.class, true);
	...
	public void testTimeZone() {
		...
		String str = Gsons.jsonify(timeZone);
		System.out.println(">>>>" + str);

		TimeZone tz = Gsons.objectify(str, TimeZone.class);
	...
	public void testLocale() {
		...
		String str = Gsons.jsonify(locale);
		System.out.println(">>>>" + str);

		Locale l = Gsons.objectify(str, Locale.class);
	...
	public void testByte() {
		Data data = new Data();
		System.out.println(">>>>>>>>" + Gsons.jsonify(data));
		Data d1 = Gsons.transform(data, Data.class);
		...
		System.out.println(">>>>>>>>" + Gsons.jsonify(data));
		Data d2 = Gsons.transform(data, Data.class);
	...
	public void testMapInteger() {
		...
		Map<String, Object> m2 = Gsons.objectify(j, Map.class);

		System.out.println(">>>>" + m2.get("a"));
		System.out.println(">>>>" + m2.get("b"));

		Assert.assertEquals(13L, m2.get("a"));
		Assert.assertEquals(99.9D, ((Double) m2.get("b")).doubleValue(), 0.1);
	...
	public void testListInteger() {
		...
		@SuppressWarnings({ "unchecked" })
		List<Object> m2 = Gsons.objectify(j, List.class);

		System.out.println(">>>>" + m2.get(0));
		System.out.println(">>>>" + m2.get(1));

		Assert.assertEquals(13L, m2.get(0));
		Assert.assertEquals(99.9D, ((Double) m2.get(1)).doubleValue(), 0.1);
