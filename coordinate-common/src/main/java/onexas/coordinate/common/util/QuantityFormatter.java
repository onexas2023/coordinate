...
public class QuantityFormatter {
	...
	public Quantity parse(final String value) {
		...
		final QuantityBaseExponent baseExponent = new QuantitySuffixFormatter().parse(suffix);
		final BigDecimal unitMultiplier = BigDecimal.valueOf(baseExponent.getBase()).pow(baseExponent.getExponent(),
	...
	private String toBase1024String(final Quantity quantity) {
		...
		return resultAndTimes.getLeft()
				+ new QuantitySuffixFormatter().format(quantity.getFormat(), exponent + resultAndTimes.getRight() * 10);
	}
	...
	private String toBase10String(final Quantity quantity) {
		...
		final Pair<Long, Integer> valueAndExponent = ensureExponentIsMultipleOf3(resultAndTimes.getLeft(),
				postFactoringExponent);
		return valueAndExponent.getLeft()
				+ new QuantitySuffixFormatter().format(quantity.getFormat(), valueAndExponent.getRight());
	}
