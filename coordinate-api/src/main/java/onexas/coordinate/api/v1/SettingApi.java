 ...
 *
 */
@Tag(description = "Setting Api V1", name = SettingApi.API_TAG)

@RequestMapping(SettingApi.API_URI)
...
public interface SettingApi extends Api {

	static public final String API_NAME = "coordinate-setting";
	static public final String API_URI = "/api/coordinate/v1/setting";
	public static final String API_TAG = API_NAME;
	public static final String API_PERMISSION_TARGET = API_NAME;

	@Operation(summary = "Get the setting", tags = {
			SettingApi.API_TAG }, security = @SecurityRequirement(name = NAME_AUTH_TOKEN))
	@ApiResponses(value = {
			@ApiResponse(responseCode = HTTP_OK, description = MSG_OK, content = @Content(schema = @Schema(implementation = USetting.class))) })
	@RequestMapping(value = "/", method = RequestMethod.GET, produces = { TYPE_APP_JSON })
	public USetting getSetting();

	@Operation(summary = "Get the server setting", tags = {
			SettingApi.API_TAG }, security = @SecurityRequirement(name = NAME_AUTH_TOKEN))
	@ApiResponses(value = {
			@ApiResponse(responseCode = HTTP_OK, description = MSG_OK, content = @Content(schema = @Schema(implementation = UServerSetting.class))) })
